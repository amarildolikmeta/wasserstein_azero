...
class Continuous_MountainCarEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action: np.ndarray):
        ...
        if bool(position >= self.goal_position and velocity >= self.goal_velocity):
            self.solved = True
            self.done = True
        ...
        info["length"] = self.timestep
        info["goal"] = self.goal_position
        info["final_state"] = self.state
    ...
    def get_reward(self, position=None, position_target=None, evaluate=False):

        if position is None: position = self.state[0]
        if position_target is None: position_target = self.goal_position

        reward = -abs(position-position_target)
    ...
    def get_S(self):
        ...
        S["current_state"] = self.state
        S["done"] = self.done
        S["solved"] = self.solved
        S["timestep"] = self.timestep
        S["nn_input"] = self.current_state_to_S()
    ...
    def set_S(self, node):
        ...
        self.state = node.S["current_state"]
